...
class Day21 : Day(21) {
    ...
    private fun findShortestSequence(code: String, robotLayers: Int): Long {
        val memo = mutableMapOf<Triple<String, Int, Boolean>, Long>()

        fun solve(sequence: String, depth: Int, isNumeric: Boolean): Long {
            ...
            for (target in sequence) {
                ...
                val minLength = moves.minOf { moveSeq ->
    ...
    private fun getOptimalMoves(from: Position, to: Position, gap: Position): List<String> {
        val dx = to.x - from.x
        val dy = to.y - from.y
